...
public class SetVarShiftRight extends SetVarAbstract {
	public SetVarShiftRight(VariableAccess variableAccess, ValueChunk val) {
	...
	public ClslValue get(ClslRuntimeEnv env) {
		return variableAccess.get(env).sr(val.get(env), true);
	}
	...
	public ExecutableChunk optimize(OptimizationEnvironment env) {
		if (env.isFirstPass)
			return new SetVarShiftRight(variableAccess.optimize(env.forValue()), (ValueChunk) val.optimize(env.forValue()));
