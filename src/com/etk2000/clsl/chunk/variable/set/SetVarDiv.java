...
public class SetVarDiv extends SetVarAbstract {
	public SetVarDiv(VariableAccess variableAccess, ValueChunk val) {
	...
	public ClslValue get(ClslRuntimeEnv env) {
		return variableAccess.get(env).div(val.get(env), true);
	}
	...
	public ExecutableChunk optimize(OptimizationEnvironment env) {
		if (env.isFirstPass)
			return new SetVarDiv(variableAccess.optimize(env), (ValueChunk) val.optimize(env.forValue()));
