...
public class SetVarBinOr extends SetVarAbstract {
	public SetVarBinOr(VariableAccess variableAccess, ValueChunk val) {
	...
	public ClslValue get(ClslRuntimeEnv env) {
		return variableAccess.get(env).bor(val.get(env), true);
	}
	...
	public ExecutableChunk optimize(OptimizationEnvironment env) {
		if (env.isFirstPass)
			return new SetVarBinOr(variableAccess.optimize(env.forValue()), (ValueChunk) val.optimize(env.forValue()));
