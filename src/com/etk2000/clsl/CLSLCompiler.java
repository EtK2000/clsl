...
public class CLSLCompiler {
				...
				for (int i = 0, j, k; i < query.length(); ++i) {
					...
					switch (c = query.charAt(i)) {
						case '"':
							...
							op = false;
							sb.append(Util.unescape(query.substring(i, j + 1)));
							i = j;
							break;
						case '\'': {
							// read whole char
							...
							String une = Util.unescape(query.substring(i, j + 1));
							if (une.isEmpty())
		...
		while (arr.get(start + n) != ExpressionType.PARENTHESIS_CLOSE && arr.get(start + n) != ExpressionType.COMMA) {
			if (arr.get(start + n) == ExpressionType.PARENTHESIS_OPEN) {
				if (res.size() > 0 && res.get(res.size() - 1) instanceof GetVar) {// function
					...
					res.add(new FunctionCall(((GetVar) res.remove(res.size() - 1)).name, args.toArray(Util.CHUNK_VALUE)));
					++n;
				}
				else {
				...
				case "(":
					if (!inChr && !inStr) {
						if (BLOCK_CHARS.indexOf(res.charAt(m.start() - 1)) != -1)
							...
							wip = new FunctionCall(res.substring(i, m.start()), readArguments(res, m).toArray(Util.CHUNK_VALUE));
							i = m.start() + 1;
						}
					}
					break;
	...
	private static ExecutableChunk[] readEffect(String res, Matcher m, String prefix) {
		...
		if (i == m.start())
			return Util.CHUNK_EXEC;
		return compile(prefix + res.substring(i, isBlock ? m.start() : m.start() + 1), SYNTAX_PATTERN.matcher(""), false).toArray(Util.CHUNK_EXEC);
	}
	...
	private static ExecutableChunk readExecutable(String res, Matcher m, boolean breakAtSemi) {
		...
		return compile(res.substring(i, m.start() + 1), (Matcher) m.toMatchResult(), false).get(0);
	}
	...
	private static ValueChunk toValue(String str) {
		if (str.length() > 1 && str.charAt(0) == '"' && str.charAt(str.length() - 1) == '"')
			return new ConstArrayChunk(Util.unescape(str.substring(1, str.length() - 1)));
			...
			switch (expression) {
				...
				default:
					switch (group) {
						// function call
						case "(":
							exec.add(new FunctionCall(res.substring(i, m.start()), readArguments(res, m).toArray(Util.CHUNK_VALUE)));
	...
	public CLSLCode compileAndOptimize(String code, boolean allowFunctions, int passes) {
		CLSLCode res = compile(code, allowFunctions);
