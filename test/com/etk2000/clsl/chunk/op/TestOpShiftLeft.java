...
public class TestOpShiftLeft {
	...
	void testOptimize() {
		...
		assertEquals(
				new OpShiftLeft(getVar, getVar),
				new OpShiftLeft(getVar, getVar).optimize(env)
		);

		// test that constants are merged
		assertEquals(
				new ConstIntChunk(VALUE_0 << VALUE_1),
				new OpShiftLeft(CHUNK_VALUE_0, CHUNK_VALUE_1).optimize(env)
		);

		// test that (X << 0) -> X
		assertEquals(
				getVar,
				new OpShiftLeft(getVar, zero).optimize(env)
		);
	}
