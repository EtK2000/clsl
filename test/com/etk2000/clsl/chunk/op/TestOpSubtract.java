...
public class TestOpSubtract {
	...
	void testGet(int op1, int op2) {
		...
		final OpSubtract op = new OpSubtract(new ConstIntChunk(op1), new ConstIntChunk(op2));
	...
	void testIO() throws IOException {
		final OpSubtract original = new OpSubtract(CHUNK_VALUE_0, CHUNK_VALUE_1);

		assertEquals(
				original,
				transmitAndReceive(original, OpSubtract::new)
		);
	}
	...
	void testOptimize() {
		...
		assertEquals(
				new OpSubtract(getVar, getVar),
				new OpSubtract(getVar, getVar).optimize(env)
		);

		// test that constants are merged
		assertEquals(
				new ConstIntChunk(VALUE_0 - VALUE_1),
				new OpSubtract(CHUNK_VALUE_0, CHUNK_VALUE_1).optimize(env)
		);

		// test that (X - 0) -> X
		assertEquals(
				getVar,
				new OpSubtract(getVar, zero).optimize(env)
		);
	}

	@Test
	void testToString() {
		final OpSubtract op = new OpSubtract(CHUNK_VALUE_0, CHUNK_VALUE_1);
	...
	void testWithFirstOp() {
		...
		final OpSubtract expected = new OpSubtract(newValue, CHUNK_VALUE_1);
